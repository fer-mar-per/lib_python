...
class Window(QWidget):
    ...
    def _add_buttons(self, addStretch):
        btn01 = QPushButton("Push Button", self)
        btn02 = QToolButton()
        btn02.setIcon(QIcon("_ico/btnFullImg.png"))
        btn02.setIconSize(QSize(48, 48))  # Height & Width
        btn02.setToolTip("Nice magic")
        btn02.setStyleSheet("border: none;")

        if addStretch:
            self._gui['lay_vbox'].addStretch()
        self._gui['lay_vbox'].addWidget(btn01)
        self._gui['lay_vbox'].addWidget(btn02)
        if addStretch:
            self._gui['lay_vbox'].addStretch()

        # Set the margins for the layout (left, top, right, bottom)
        self._gui['lay_vbox'].setContentsMargins(120, 20, 10, 30)
